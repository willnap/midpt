  ...
  getCentroid: (req, res, next) => {
      ...
      for (let j = 0; j < 2; j++) {
        promArr.push(
          new Promise((resolve, reject) => {
            ...
            &destination=${res.locals.midpt[i].lat},${res.locals.midpt[i].lng}
    ...
    Promise.all(promArr).then(() => {
      ...
      res.locals.isoIntersectionPoints =
        res.locals.isoIntersectionPoints[bestTracker];
      console.log(
        'all midpts ! ',
        res.locals.midpt,
        'best midpt: ',
        res.locals.midpt[bestTracker]
      ); //for testing
      res.locals.midpt = res.locals.midpt[bestTracker];
      next();
    });
  }
};
